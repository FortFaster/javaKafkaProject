...
public class Consumer implements Runnable {
	...
	public void run() {
		...
		System.out.println("Consumer Run No"+theadNo);
		
        KafkaConsumer consumer = new KafkaConsumer(KafkaConstant.kafkaaPropConsumers());
        List topics = new ArrayList();
        topics.add(KafkaConstant.test_topic);
        topics.add(KafkaConstant.test_topic1);
        ...
        try{
        	while (true) {
				...
				for (ConsumerRecord<String, String> record : records) {
					...
					System.out.println("TheadNo"+theadNo+" :No"+count+" :parameters==>"+parameters+" topic==>"+record.topic());
//					try {
